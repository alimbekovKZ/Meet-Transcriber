...
// Add this handler to background.js

// Handle tab capture requests
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "requestTabCapture") {
        // We need the tab ID to capture
        const tabId = sender.tab?.id;
        
        if (!tabId) {
            sendResponse({ error: "Не удалось определить ID вкладки" });
            return true;
        }
        ...
        chrome.tabCapture.capture({
            ...
            if (chrome.runtime.lastError) {
                sendResponse({ 
                    error: chrome.runtime.lastError.message || "Ошибка при захвате вкладки" 
                });
                return;
            }
            ...
            if (!stream) {
                sendResponse({ 
                    error: "Поток не получен. Возможно, отсутствуют необходимые разрешения." 
                });
                return;
            }
            
            // Need to transfer the MediaStream to the content script
            // This is a limitation in Chrome Extensions - we can't directly transfer MediaStream
            // Instead, we'll need the content script to use getDisplayMedia instead
            
            // Stop the stream since we can't use it directly
            stream.getTracks().forEach(track => track.stop());
            
            sendResponse({ 
                success: false,
                needUserGesture: true,
                message: "Требуется явное разрешение пользователя на доступ к аудио" 
            });
        });
